...
import { NinjaColor, OPPONENTS, Opponent } from '../types';
import { getSenseiWisdom } from '../services/geminiService';

interface MenuProps {
  onStart: (selectedColor: NinjaColor, opponent: Opponent) => void;
}

const Menu: React.FC<MenuProps> = ({ onStart }) => {
  const [wisdom, setWisdom] = useState<string>("Loading Sensei's wisdom...");
  const [step, setStep] = useState<'HERO' | 'VILLAIN'>('HERO');
  ...
  const renderCharacterCard = (
    ...
    <button
      ...
      className={`
        ...
        ${isSelected 
          ? `border-${isVillain ? 'purple' : 'yellow'}-400 bg-gray-700 scale-110 shadow-[0_0_25px_rgba(${isVillain ? '147,51,234' : '250,204,21'},0.4)] z-20` 
      ...
      <div className="relative w-20 h-24 mb-2 flex flex-col items-center">
          ...
          >
              ...
              <div className="absolute top-3 left-1 right-1 h-4 bg-yellow-400 rounded-sm flex items-center justify-center gap-2 shadow-inner">
                  {/* Eyes - Red for villains, Black for heroes */}
                  <div className={`w-1.5 h-1.5 rounded-full transition-transform duration-300 ${isSelected ? 'scale-110' : ''} ${isVillain ? 'bg-red-600' : 'bg-black'}`}></div>
                  <div className={`w-1.5 h-1.5 rounded-full transition-transform duration-300 ${isSelected ? 'scale-110' : ''} ${isVillain ? 'bg-red-600' : 'bg-black'}`}></div>
          ...
          >
              ...
              {/* Emblem */}
              <div className={`mt-3 w-5 h-5 rounded-full ${isVillain ? 'bg-purple-900 border-purple-500' : 'bg-yellow-400 border-yellow-600'} border-2 shadow-sm flex items-center justify-center transition-transform duration-500 ${isSelected ? 'rotate-180' : ''}`}>
                   <div className={`w-2 h-2 rounded-full opacity-80 ${isVillain ? 'bg-red-500' : 'bg-yellow-600'}`}></div>
              </div>
              ...
              {/* Arms hint */}
              <div className="absolute -left-1 top-2 w-2 h-8 rounded-l-md brightness-90" style={{ backgroundColor: color }}></div>
              <div className="absolute -right-1 top-2 w-2 h-8 rounded-r-md brightness-90" style={{ backgroundColor: color }}></div>
          </div>
      </div>

      <span className={`font-bold text-lg mt-2 ${isSelected ? (isVillain ? 'text-purple-400' : 'text-yellow-400') : 'text-gray-200'}`}>{name}</span>
      <span className="text-xs text-gray-400 uppercase tracking-wider">{element}</span>
      
      {/* Selection Indicator Glow */}
      {isSelected && (
          <div className={`absolute inset-0 rounded-xl bg-gradient-to-t ${isVillain ? 'from-purple-500/10' : 'from-yellow-500/10'} to-transparent pointer-events-none animate-pulse`}></div>
      )}
    </button>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full bg-slate-900 text-white p-8 relative overflow-hidden">
      ...
      <div className={`absolute top-[-20%] left-[-10%] w-96 h-96 rounded-full blur-3xl opacity-20 animate-pulse transition-colors duration-1000 ${step === 'HERO' ? 'bg-red-600' : 'bg-purple-900'}`} />
      <div className={`absolute bottom-[-20%] right-[-10%] w-96 h-96 rounded-full blur-3xl opacity-20 animate-pulse delay-700 transition-colors duration-1000 ${step === 'HERO' ? 'bg-blue-600' : 'bg-green-900'}`} />
      ...
      {step === 'HERO' && (
        <>
            <div className="bg-gray-800/80 p-6 rounded-lg border border-gray-600 max-w-2xl text-center mb-8 shadow-xl backdrop-blur-sm z-10 animate-fade-in">
                <p className="italic text-yellow-100 text-lg">"{wisdom}"</p>
                <p className="text-right text-sm text-gray-400 mt-2">- Sensei Wu</p>
            </div>
            
            <h3 className="text-2xl font-bold mb-6 text-white z-10">Choose Your Ninja</h3>
            
            <div className="grid grid-cols-2 md:grid-cols-5 gap-6 mb-10 z-10">
                {ninjas.map((ninja) => renderCharacterCard(
                    ninja.color, 
                    ninja.color, 
                    ninja.name, 
                    ninja.element, 
                    selectedNinja === ninja.color, 
                    () => setSelectedNinja(ninja.color)
                ))}
            </div>

            <button
                onClick={() => setStep('VILLAIN')}
      ...
      {step === 'VILLAIN' && (
        <>
             <div className="bg-gray-800/80 p-6 rounded-lg border border-purple-500/50 max-w-2xl text-center mb-8 shadow-xl backdrop-blur-sm z-10 animate-fade-in">
                <p className="italic text-purple-200 text-lg">"The darkness rises... choose your destiny!"</p>
            </div>

            <h3 className="text-2xl font-bold mb-6 text-purple-400 z-10">Choose Opponent</h3>

            <div className="grid grid-cols-2 md:grid-cols-5 gap-6 mb-10 z-10">
                {OPPONENTS.map((opp) => renderCharacterCard(
                    opp.id,
                    opp.color,
                    opp.name,
                    opp.element,
            ...
            <div className="flex gap-4 z-10">
                <button
                    ...
                    className="px-8 py-4 bg-gray-700 rounded-full text-xl font-bold shadow-lg hover:bg-gray-600 transition-all uppercase tracking-wider"
                >
                    Back
                </button>
                <button
                    onClick={() => onStart(selectedNinja, selectedOpponent)}
                    className="px-12 py-4 bg-gradient-to-r from-purple-600 to-red-600 rounded-full text-2xl font-bold shadow-lg hover:shadow-purple-500/50 hover:scale-105 transition-all active:scale-95 uppercase tracking-wider"
                >
                    FIGHT!
                </button>
            </div>
